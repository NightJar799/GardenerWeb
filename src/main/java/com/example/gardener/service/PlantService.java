...
public class PlantService {
    ...
    public PlantListDTO mapPlantListDTO(Growth growth, Plant plant) {
        return new PlantListDTO(plant.getId(), plant.getName(), plant.getScienceName(),
                growth.getClimate(), growth.getSpace(), growth.getSoil(),
                growth.getWater(), growth.getLandScape());
    }
    ...
    private PlantDTO mapToPlantDTO(Plant plant, Growth growth, BioChar bioChar){
        return new PlantDTO(plant.getId(), bioChar.getLeafType(), bioChar.getRoot(),
                bioChar.getFruit(), bioChar.getAmmFruit().toString(), bioChar.getMorphology(),
                growth.getPpfd(), growth.getHumidity(), growth.getPh(), growth.getSpace(),
                growth.getSoil(), growth.getSurvivability(), growth.getGrowthSpeed(),
                growth.getClimate(), growth.getWater(), growth.getLandScape(),
                plant.getName(), plant.getScienceName(), plant.getDescription());
    }

    @Cacheable(value = "plantDetails", key = "#id", unless = "#result == null")
    public PlantDTO getPlantDTO(Integer id) {
        return mapToPlantDTO(plantRepository.getReferenceById(id),
                growthRepository.getReferenceById(id),
                bioCharRepository.getReferenceById(id));
    }

    @Cacheable(value = "allPlants", key = "'list'", unless = "#result.isEmpty()")
    public List<PlantListDTO> getPlantAllForListDTO() {
        ...
        for (Growth growth : growthChar) {
            plantListDTOS.add(mapPlantListDTO(growth,
